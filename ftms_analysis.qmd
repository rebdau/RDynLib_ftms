...
sample_data_ftms <- read_xlsx("C:/Users/amentag/Desktop/these/These/Ahlam/XCMS/flaxseed/ftms_data_files.xlsx") |>
                    as.data.frame()
file <- "C:/Users/amentag/Desktop/these/These/Ahlam/XCMS/flaxseed/S_all/"
ftms0 <- readMsExperiment(paste0(file,sample_data_ftms$data_file), sampleData = sample_data_ftms)
ftms0
```
...
```{r}
bpc <- chromatogram(ftms, aggregationFun = "max")
plot(bpc, col = col_sample_ftms)
```
...
now we filter empty spectra

```{r}
ftms <- filterSpectra(ftms, filterEmptySpectra)
msLevel(spectra(ftms0)) |>
  table()
```
...
![](http://127.0.0.1:24279/graphics/bb2798e2-7ba2-43bf-b51f-bb1f95c45d6e.png)
...
![](http://127.0.0.1:24279/graphics/6d4ed144-479a-4398-b778-fcc23b080562.png)
...
![](http://127.0.0.1:24279/graphics/3e8cd59b-7136-464b-b5f9-1fcb3ad07600.png)
...
![](http://127.0.0.1:24279/graphics/77817158-2ace-4084-b032-da19663540bc.png)
...
![](http://127.0.0.1:24279/graphics/423b3ed6-5cf4-455e-adea-6fbf2648707f.png)

# Data preprocessing

here we take an my range of an known compound
...
this data is on profile mode we need to centroid it.
...
![](http://127.0.0.1:24279/graphics/0b912f46-8a08-4e35-841d-f4be40c0581b.png)
...
![](http://127.0.0.1:24279/graphics/ffaf9264-1fca-47d9-b6f6-60dafc12a732.png)
...
![](http://127.0.0.1:24279/graphics/9a4dc85e-514e-4b0a-a5e6-3155771b3167.png){width="517"}
...
![](http://127.0.0.1:24279/graphics/760c61f9-955f-4eaa-b26d-07078eb31425.png)

we can see from the plot above that there is one sample that has been largely adjusted
...
![](http://127.0.0.1:24279/graphics/c75b1cb2-b186-4aee-80c7-586a0c4d9660.png)
...
![](http://127.0.0.1:24279/graphics/596dfa3e-e07e-4f1f-8d78-f9426c3ad4e0.png)
...
![](http://127.0.0.1:24279/graphics/8a44aaeb-3b1a-49e2-88e9-c541c39ea10e.png)
...
![](http://127.0.0.1:24279/graphics/83171874-8f88-41fc-b708-ea265391bfc0.png)
...
![](http://127.0.0.1:24279/graphics/dd1ad109-8e1e-45ce-92db-96c3b285a4dd.png)
...
![](http://127.0.0.1:24279/graphics/579cacb3-fab8-4502-aefa-639991a59b67.png)
...
![](http://127.0.0.1:24279/graphics/5325675c-4417-4ad5-9819-3d76f0d43e5c.png)
...
![](http://127.0.0.1:24279/graphics/81c77abe-1edd-4726-aa6f-8efe03649198.png)
...
![](http://127.0.0.1:24279/graphics/2c299f13-a241-426e-aaea-77d0b5f6d227.png)
...
![](http://127.0.0.1:24279/graphics/310763f6-a6f4-4009-8fc8-c12304f5f60b.png)
...
save(ftms, file = "C:/Users/amentag/Desktop/Rcodes/ftms.RData")
```
...
![](http://127.0.0.1:24279/graphics/4083c5c9-bc86-4bc3-ba1c-9598f2834b8d.png)
...
![](http://127.0.0.1:24279/graphics/91390015-854c-49dd-a6f7-0c7931a93d94.png)
...
![](http://127.0.0.1:24279/graphics/4aafc1b5-eeac-48bb-8ced-7fdf17124a3a.png)
...
![](http://127.0.0.1:24279/graphics/3476546c-35e1-42d6-80e7-cc7237a19024.png)

Let's try (5,20) as a peak width parameter instead of (20,80)

```{r}
a_kn3 <- findChromPeaks(a_kn, CentWaveParam(peakwidth = c(5, 20), integrate = 2))

```

```{r}
pks <- chromPeaks(a_kn3)
head(pks)
quantile(unname(pks[, "rtmax"] - pks[, "rtmin"]))
```
...
par(mfrow = c(1, 2))
#' In one sample
plot(a_kn3[, 1], xlim = c(1390, 1450))
#' In all samples
plot(a_kn3[, 1:5], xlim = c(1390, 1450))
```

![](http://127.0.0.1:24279/graphics/f7bc04e5-7f2a-487f-9026-6b8d6676c7b9.png)

```{r}
plot(a_kn3[, 1], xlim = c(870, 1000))
plot(a_kn3[, 1:5], xlim = c(870, 1000))
```

![](http://127.0.0.1:24279/graphics/bddd7dfb-3ee9-405b-80c2-f64bc055282f.png)

```{r}
ftms2 <- findChromPeaks(ftms, CentWaveParam(peakwidth = c(5, 20), integrate = 2),
                       chunkSize = 2)
```

```{r}
pks3 <- chromPeaks(ftms2)
quantile(unname(pks3[, "rtmax"] - pks3[, "rtmin"]))
quantile(unname(pks3[, "mzmax"] - pks3[, "mzmin"]))
```

```{r}
mnpp2 <- MergeNeighboringPeaksParam(expandRt = 5.79, expandMz = 0.0001)
ftms2 <- refineChromPeaks(ftms2, mnpp2)
```
...
pdp <- PeakDensityParam(sampleGroups = sampleData(ftms)$sample_id,
                        bw = 6, minFraction = 0.5)
ftms2 <- groupChromPeaks(ftms2, param = pdp)
```
...
ftms2 <- adjustRtime(ftms2, param = pgp)
```

```{r}
plotAdjustedRtime(ftms2, col = paste0(col_sample_ftms, 80))
```

![](http://127.0.0.1:24279/graphics/48ab8eb7-8616-48d7-b68a-c47633cce3bf.png){width="501"}

```{r}
bpc_adj2 <- chromatogram(ftms2, aggregationFun = "max", chromPeaks = "none")
a_adj2 <- chromatogram(ftms2, aggregationFun = "max", mz = mzr)
```
...
![](http://127.0.0.1:24279/graphics/e1cfe032-a5aa-4ac9-8205-b336aa3591fb.png)

```{r}
par(mfrow = c(1, 1))
plot(bpc_adj2, col = paste0(col_sample_ftms, 80))
grid()
```

![](http://127.0.0.1:24279/graphics/6c7edd22-3e73-4eb7-bcee-9a056e88ccdf.png)

```{r}
par(mfrow = c(1, 1))
plot(bpc, col = paste0(col_sample_ftms, 80), xlim = c(400, 500))
grid()
```

![](http://127.0.0.1:24279/graphics/46e720a9-291f-4952-ade2-0049bc92edb5.png)

```{r}
par(mfrow = c(1, 1))
plot(bpc_adj2, col = paste0(col_sample_ftms, 80), xlim = c(400, 500))
grid()
```

![](http://127.0.0.1:24279/graphics/cc7ee34c-0066-444e-b162-4a59cfe785e4.png)

```{r}
par(mfrow = c(2, 1))
plot(a, col = paste0(col_sample_ftms, 80))
grid()
plot(a_adj2, col = paste0(col_sample_ftms, 80))
grid()

```

![](http://127.0.0.1:24279/graphics/47179c5d-2974-425d-9249-fe5d3fc63851.png)

```{r}
#' Configure settings
pdp <- PeakDensityParam(sampleGroups = sampleData(ftms2)$sample_id,
                        bw = 2, minFraction = 0.5)
...
![](http://127.0.0.1:24279/graphics/dd768b4c-8a0c-4481-a0c9-13392213097d.png)
