...
In this quarto document we create an assembled MS/MS data from ftms object that contains one tree per feature created in [Select Spectral Trees](ftms_filtering).
...
Using the `RDynLib::peaks_round_mz()` function, we round `mz` values to the first integer and keep only the most intense peak if rounding results in duplicated mz.
