---
title: "Create flattened pseudo-MS/MS spectra from multilevel MSn data"
format:
  html:
    toc: true
    self-contained: true
...
In this quarto document we create flattened pseudo-MS/MS spectra from a `Spectra` object containing one MSn tree per feature, with up to four fragmentation levels (MS2-MS4). The \`Spectra object was created in the [ftms_filtering.qmd](https://github.com/rebdau/RDynLib_ftms/blob/ahlam/ftms_filtering.qmd) file.

# Data import

Loading all required libraries and importing the results from the ftms filtered object.
...
First we load the `Spectra` ftms_one_tree object with one representative tree per feature.
...
Next we round `mz` values to the first integer using `round_perl()` function and keep only the most intense peak if rounding results in duplicated mz.
