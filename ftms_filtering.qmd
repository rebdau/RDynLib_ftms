...
```{r}
load(file.path(pth, "data", "ftms_sh4_minfr.RData"))
class(ftms)
```
...
it returns 3882 MS level 2 spectra. The total number of features for
...
In total, there were 20804 MS2 spectra. This means that 81% of the MS2
spectra ((20804-3882)\*100/20804) could not be assigned to any
...
assigned to two features (314 MS2 spectra). MS2 spectra are maximally
assigned to 3 features (27 MS2 spectra).
...
ids2 <- c("FT1526", "FT1529")
a_chr2 <- featureChromatograms(ftms, features = ids2)

plot(a_chr2[1])
abline(v = 3088.41, col = "red", lty = 2)

plot(a_chr2[2])
abline(v = 3371.06, col = "red", lty = 2)
...
hist(mzmed_diff, xlim = c(0,0.005), breaks = 1000, main = "Distribution of feature_mzmed range per ms2_id")
par(opar)
...
```{r}
# Identify ms2_id with feature_mzmed range > 0.001
significant_ms2 <- mzmed_diff[mzmed_diff > 0.001]
...
ms2_id_non_significant <- names(mzmed_diff[mzmed_diff <= 0.001])
ms2_id_significant <- names(mzmed_diff[mzmed_diff > 0.001])
...
==\> 76.5% of 5% is ~3.8 % of the ambiguously assigned MS2 spectra that can
