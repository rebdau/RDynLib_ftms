...
This Quarto document presents the analysis of FTMS LC-MSn flaxseed data for selecting one representative spectral tree per feature. Preprocessing and feature definition was performed in [Preprocessing steps](ftms_xcms_short.qmd).

# Data import

Installing the latest development version of RDynLib_2.0.0.

```{r}
# Check if RDynLib is installed
pkg_installed <- "RDynLib" %in% rownames(installed.packages())
if (!pkg_installed) {
  message("Installing RDynLib dev branch from GitHub...")
  devtools::install_github("rebdau/RDynLib_SQL@dev", upgrade = "never", quiet = TRUE)
}
library(RDynLib)
```
...
```{r}
ftms_msn_tree <- RDynLib:::ftms_all_levels(ftms,ms2)
...
```{r}
ftms_msn_tree <- RDynLib:::assign_msntree_id(ftms_msn_tree)
