...
The percentage of features with at least one MS2 is 7.57%
...
the percentage of MS2 spectra that are assigned to a feature is 9.88%
...
```{r}
a <- ms2[ms2_id == ms2_id_significant[1L]]
print(spa <- spectraData(a))
```
we have just one MS2 spectra that was assigned to 2 different features

bellow the Chromatograms of those features 
```{r}
ids2 <- c("FT0392", "FT0397")
a_chr2 <- featureChromatograms(ftms, features = ids2)

plot(a_chr2[1])
abline(v = 135.519, col = "red", lty = 2)

plot(a_chr2[2])
abline(v = 144.079, col = "red", lty = 2)
```
...
```{r}
select_unique_MS2 <- function(ms2, ftms, threshold) {
  ...
  for (gname in names(group_idx)) {
    ...
    # precursor m/z of the MS2 spectra
    ...
    mz_features <- featureDefinitions(ftms)[feature_ids, "mzmed"]

    # remove assignments if |precursorMz - feature_mzmed| > 0.002
    mz_diff <- abs(ms2_mz - mz_features)
    valid_features <- feature_ids[mz_diff <= threshold]

    # If no feature passes the mz filter so mark all as ambiguous
    if (length(valid_features) == 0) {
      final_feature[[gidx[1]]] <- character(0)
      amb_f[[gidx[1]]] <- feature_ids
      next  # go to the next group...
    }

    # Initialisation
    keep_feat <- character(0) #features to keep
    amb_feat <- valid_features #ambiguous features

    # Loop on each feature in the group
    for (i in seq_along(valid_features)) {
      i_id <- valid_features[i]
      ...
      num <- abs(ms2_rt - rt_features[feature_ids == i_id])
      ratios_rt <- sapply(seq_along(valid_features)[-i], function(j) {
        denom <- abs(ms2_rt - rt_features[feature_ids == valid_features[j]])
...
ms2 <- select_unique_MS2(ms2, ftms, threshold = 0.002) 
```
...
The total number of MSn trees is 2000:
...
within dataOrigin S1R1, depending on MS2 with ScanIndex 2532 which was
assigned to one FeatureID: FT1159 Within dataOrigin S1R1, scanIndex 2533
(MS3), 2534 (MS3), and 2535 (MS4) are assigned to a single feature:
FT2705.
...
The number of trees that go up to MS2 level is : 559 The number of
trees that go up to MS3 level is : 382 The number of trees that go up to
MS4 level is : 1059
...
we have 518 unique feature and unique MSntreeID, That's mean that we
have for each feature one representative tree.

Now we save the resulting ftms spectra object to a local directory.

```{r}
save(ftms_one_tree, file = file.path(pth, "data", "1_tree_ftms.RData"))
```
