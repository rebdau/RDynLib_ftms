...
```{r}
ms2_S1R4 <- spectraData(ms2)[ spectraData(ms2)$scanIndex == 2864 &
...
```{r}
peaksData(ms2)[spectraData(ms2)$scanIndex == 4356 &
                  spectraData(ms2)$dataOrigin == "C:\\Users\\amentag\\Desktop\\Rcodes\\RDynLib_ftms\\data\\mzML_cent1\\S1R3.mzML",][[1,]]

```

```{r}
FT5341 <- spectraData(ms2)[ spectraData(ms2)$feature_id == "FT5490",]
...
#Select the 25% of spectra with the highest peaks number for each feature

```{r}

Highest_peaks_nb <- function(ms2, percentage = 34) {
  n_peaks <- lengths(peaksData(ms2))
  fid <- spectraData(ms2)$feature_id
  
  keep_idx <- unlist(lapply(split(seq_along(ms2), fid), function(idx) {
    n <- ceiling(length(idx) * percentage / 100)
    idx[order(n_peaks[idx], decreasing = TRUE)[seq_len(n)]]
  }))
...
ms2_H <- Highest_peaks_nb(ms2, 34)

spectraData(ms2_H)[ spectraData(ms2_H)$feature_id == "FT5229",]
```
...
ftms_msn_tree <- ftms_all_levels(ftms,ms2_H)
...
```{r}
save(ftms_one_tree, file = file.path(pth, "data", "ftms_one_tree_test.RData"))
```
...
```{r}
tst <- spectraData(ftms_one_tree)[spectraData(ftms_one_tree)$feature_id == "FT5490",]
tst
```

```{r}
peaksData(ftms_one_tree)[spectraData(ftms_one_tree)$feature_id == "FT5490" & spectraData(ftms_one_tree)$msLevel == 2,][[1,]]
```
```{r}
...
```{r}
spectraData(ftms_one_tree)[spectraData(ftms_one_tree)$feature_id == "FT4553", ]
