...
```{r}
load(file.path(pth, "data", "ftms_fin6.RData"))
class(ftms)
```
...
it returns 3551 MS level 2 spectra. The total number of features for
...
In total, there were 20904 MS2 spectra. This means that 83% of the MS2
spectra ((20904-3551)\*100/20904) could not be assigned to any
...
The percentage of features with at least one MS2 is 8.32%
...
the percentage of MS2 spectra that are assigned to a feature is 15.52%
...
assigned to two features (274 MS2 spectra). MS2 spectra are maximally
assigned to 3 features (22 MS2 spectra).
...
feature, 12% were assigned to multiple features. Of all MS2 spectra that
were assigned to multiple features, 12% were assigned to
features that differ significantly in mzmed. Of these, 92.3% could be
assigned unambiguously to a single feature based on the difference
precursorMz – feature_mzmed < 0.001

==\> 2.3% of 12% is ~11.1% of the ambiguously assigned MS2 spectra that can
be resolved based on precursorMz – feature_mzmed < 0.001.
